...
void DisplayHelp() {
    ...
    printf("man : To Display manual of command\n");
...
int LseekFile(int fd, int size, int from) {
    ...
    if ((UFDTArr[fd].ptrfiletable->mode == READ) || (UFDTArr[fd].ptrfiletable->mode == READ + WRITE)) {
        ...
        if (from == START) {
            if (size > UFDTArr[fd].ptrfiletable->ptrinode->FileSize || size < 0)
                return -1;
            if (size > UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)
                UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize = size;
            UFDTArr[fd].ptrfiletable->writeoffset = size;
        } else if (from == CURRENT) {
            if (((UFDTArr[fd].ptrfiletable->writeoffset) + size) > UFDTArr[fd].ptrfiletable->ptrinode->FileSize || ((UFDTArr[fd].ptrfiletable->writeoffset) + size) < 0)
                return -1;
            if (((UFDTArr[fd].ptrfiletable->writeoffset) + size) > UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)
                UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize = UFDTArr[fd].ptrfiletable->writeoffset + size;
            UFDTArr[fd].ptrfiletable->writeoffset = UFDTArr[fd].ptrfiletable->writeoffset + size;
        } else if (from == END) {
            if ((UFDTArr[fd].ptrfiletable->ptrinode->FileSize + size) > UFDTArr[fd].ptrfiletable->ptrinode->FileSize || (UFDTArr[fd].ptrfiletable->ptrinode->FileSize + size) < 0)
                return -1;
